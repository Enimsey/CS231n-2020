...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        correct_classes_exp_probabilities = probabilities[np.arange(N), y]
        ...
        # remove 1 for the correctly predicted classes to stick with the formula
        probabilities[np.arange(N), y] -= 1
        probabilities /= N
        dW2 = H1.T.dot(probabilities)
        dW2 += 2*reg*W2
        db2 = np.sum(probabilities, axis=0)
        grads['W2'] = dW2
        grads['b2'] = db2

        hidden = probabilities.dot(W2.T)
        hidden[H1==0] = 0
        db1 = np.sum(hidden, axis=0)
        dW1 = X.T.dot(hidden) + 2*reg*W1
    ...
    def predict(self, X):
        ...
        y_pred = np.argmax(H1.dot(self.params['W2']) + self.params['b2'],axis = 1)
