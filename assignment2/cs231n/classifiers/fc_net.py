...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grads['W2'] = out.T.dot(dx)  + self.reg * W2
        grads['b2'] = dx.T.dot(np.ones(out.shape[0]))
        
        dx2 = dx.dot(W2.T) # gradient with regard to x2 = W1*X + b1
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for l in range(self.num_layers):
            self.params['W'+str(l+1)] = weight_scale * np.random.randn(all_dims[l],all_dims[l+1])
            self.params['b'+str(l+1)] = np.zeros(all_dims[l+1])
